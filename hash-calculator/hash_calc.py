...
def generate_all_hashes(text: str, salt: str = "") -> dict:
    """
    Verilen metin ve isteğe bağlı salt (tuz) için tüm temel kriptografik hash özetlerini üretir.
    """
    if not text:
        text = ""
    ...
    # NTLM (Windows SAM Hash - UTF-16LE MD4)
    try:
...
def identify_hash_type(hash_str: str) -> dict:
    """
    Bilinmeyen bir hash dizisinden (örn: 5e884898da28...) hash algoritma türünü otomatik tespit eder.
    """
    ...
    # Özel Format Kontrolleri (bcrypt, Modular Crypt Format vb.)
...
def verify_hash_match(plain_text: str, target_hash: str, salt: str = "") -> dict:
    """
    Verilen açık metin ile hedef hash'in eşleşip eşleşmediğini doğrular.
    """
