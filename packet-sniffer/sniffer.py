...
def parse_ip_header(raw_data):
    """
    IPv4 Başlığını Unpack (İkili Çözümleme) Eder.
    IPv4 Başlığı en az 20 bayttır.
    Format: !BBHHHBBH4s4s
    """
    if len(raw_data) < 20:
        return None
    ...
    ihl = (version_ihl & 0xF) * 4  # Header uzunluğu (bayt cinsinden)
...
def parse_tcp_header(raw_data):
    """
    TCP Başlığını Unpack Eder (En az 20 bayt).
    Format: !HHLLBBHHH
    """
    if len(raw_data) < 20:
        return None
...
def parse_udp_header(raw_data):
    """
    UDP Başlığını Unpack Eder (8 bayt).
    Format: !HHHH
    """
    if len(raw_data) < 8:
        return None
...
def parse_icmp_header(raw_data):
    """
    ICMP Başlığını Unpack Eder (4 bayt).
    Format: !BBH
    """
    if len(raw_data) < 4:
        return None
...
def format_hex_ascii(data):
    """
    Bayt dizisini Wireshark/xxd tarzı Hex ve ASCII dökümüne çevirir.
    """
    if not data:
        return "Payload boş"
    
    lines = []
    for i in range(0, min(len(data), 256), 16):  # Maksimum 256 bayt göster
        chunk = data[i:i+16]
...
def generate_sample_packets(count=5):
    """
    Sistem yöneticisi yetkisi olunmadığında veya test amaçlı örnek ağ paketleri üretir.
    """
